...
public class HookUtil {
	...
	public void registerOnLoadHooks(){
		onLoadHooks.forEach(this::addHook);
	}
	...
	public boolean removeHook(String hookName) {
		...
		for (HookPurpose sup : usedHooks.keySet()){
			...
			// equals unnecessary ?
			rem.addAll(present.stream().filter(ref -> ref == hook || ref.getHookName().equals(hookName)).collect(Collectors.toList()));
	...
	public final List<Hook> getUsedHooks(final HookPurpose purpose){
		...
		if (hooks == null) return new LinkedList<>();
		return hooks;
	}
